...
From the sample data, we can calculate a \emph{statistic}. A statistic is a number that is a property of the sample. For example, if we consider one math class to be a sample of the population of all math classes, then the average number of points earned by students in that one math class at the end of the term is an example of a statistic. The statistic can be used as an estimate of a population \emph{parameter}. A parameter is a number that is a property of the population. Since we considered all math classes to be the population, then the average number of points earned per student over all the math classes is an example of a parameter.
...
Two words that come up often in statistics are \emph{average} and \emph{proportion}. If you were to take three exams in your math classes and obtained scores of 86, 75, and 92, you calculate your average score by adding the three exam scores and dividing by three (your average score would be 84.3 to one decimal place). If, in your math class, there are 40 students and 22 are men and 18 are women, then the proportion of men students is \(22/40\) and the proportion of women students is \(18/40\). Average and proportion are discussed in more detail in later chapters.
...
The \texttt{R} Programming Environment is a widely used open source system for statistical analysis and statistical programming. It includes thousands of functions for the implementation of both standard and exotic statistical methods and it is probably the most popular system in the academic world for the development of new statistical tools. We will use \texttt{R} in order to apply the statistical methods that will be discussed in the book to some example data sets and in order to demonstrate, via simulations, concepts associated with probability and its application in statistics.

The demonstrations in the book involve very basic \texttt{R} programming skills and the applications are implemented using, in most cases, simple and natural code. A detailed explanation will accompany the code that is used.

Learning \texttt{R}, like the learning of any other programming language, can be achieved only through practice. Hence, we strongly recommend that you not only read the code presented in the book but also run it yourself, in parallel to the reading of the provided explanations. Moreover, you are encouraged to play with the code: introduce changes in the code and in the data and see how the output changes as a result. One should not be afraid to experiment. At worst, the computer may crash or freeze. In both cases, restarting the computer will solve the problem \ldots{}

You may download \texttt{R} from the \texttt{R} project home page~\url{https://www.r-project.org} and install it on the computer that you are using.

In addition to installing \texttt{R} we recommend that you also install \texttt{RStudio}~\url{https://www.rstudio.com}. RStudio is an integrated development environment (IDE) for \texttt{R}. It includes a console, syntax-highlighting editor that supports direct code execution, as well as tools for plotting, history, debugging and workspace management.

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{_figures/rstudio} 

}

\caption{The RStudio interface}\label{fig:fig1b}
\end{figure}

The screenshot in Figure~\ref{fig:fig1b} shows the RStudio interface. There are four panels. The top-left panel is the editor, the bottom-left panel shows the \texttt{R-Console}. The two right panels show the R-environment, history, file browser, plot panel and help viewer among others.
...
\texttt{R} is an object-oriented programming system. During the session you may create and manipulate objects by the use of functions that are part of the basic installation. You may also use the \texttt{R} programming language. Most of the functions that are part of the system are themselves written in the \texttt{R} language and one may easily write new functions or modify existing functions to suit specific needs.
...
Notice that the output of the function ``\texttt{table}'' is a table of the different levels of the input vector and the frequency of each level. This output is yet another type of an object.

The bar-plot of Figure~\ref{fig:intror1} can be produced by the application of the function ``\texttt{plot}'' to the object that is produced as an output of the function ``\texttt{table}'':
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-13}{}{\label{exr:unnamed-chunk-13} }The number of customers that wait in front of a coffee shop at the opening was reported during 25 days. The results were:
...
After saving a file with the data in a directory, \texttt{R} should be notified where the file is located in order to be able to read it. A simple way of doing so is by setting the directory with the file as \texttt{R}'s \emph{working directory}. The working directory is the first place \texttt{R} is searching for files. Files produced by \texttt{R} are saved in that directory. In \texttt{RStudio} one may set the working directory of the active \texttt{R} session to be some target directory in the ``\texttt{Files}'' panel The dialog is opened by clicking on ``\texttt{More}'' on the left hand side of the toolbar on the top of the \texttt{Files} panel. In the menu that opens selecting the option of ``\texttt{Set\ As\ Working\ Directory}'' will start the dialog. (See Figure~\ref{fig:Data2}.) Browsing via this dialog window to the directory of choice, selecting it, and approving the selection by clicking the ``\texttt{OK}'' bottom in the dialog window will set the directory of choice as the working directory of \texttt{R}.
...
A full statistical ananlysis will typically involve a non-negligible number of steps. Retracing these steps in retrospect is often difficult without proper documentation. So while working from the R-console is a good way to develop (steps) in an analysis, you will need a way to document your work in order for your the analysis to be reproducible. Rather than to work only from the R-console (and change the working directory every time that \texttt{R} is opened manually), it is better to organize your R-code in so-called scripts. R-scripts are plain text files which contain the R-commands that perform your analysis. Executing these scripts will perform your full analysis and thereby make your work reproducible.

Figure~\ref{fig:Data3} shows how to create an R-script in RStudio. This is done by clicking on the first button on the main toolbar in RStudio. This will popup a menu where you can click on \texttt{R-script}. RStudio will then create a new empty R-script in the editing panel as shown in Figure~\ref{fig:Data4}. Here you can add the R-commands that will perform your analysis. Make sure to save your new script. Finally note that you can select one or several lines in your R-script and click on the `Run' button in the toolbar to execute them in the R-console.

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{_figures/create-r-script} 

}

\caption{Creating an R-script}\label{fig:Data3}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{_figures/new-r-script} 

}

\caption{Editing an R-script}\label{fig:Data4}
\end{figure}

In the rest of this book we assume that a designated directory is set as \texttt{R}'s working directory and that all external files that need to be read into \texttt{R}, such as ``\texttt{ex1.csv}'' for example, are saved in that working directory. Once a working directory has been set then the history of subsequent \texttt{R} sessions is stored in that directory. Hence, if you choose to save the image of the session when you end the session then objects created in the session will be uploaded the next time the \texttt{R\ Console} is opened.
...
The distinction between continuous and discrete numeric data is not reflected usually in the statistical method that are used in order to analyze the data. Indeed, \texttt{R} does not distinguish between these two types of numeric data and store them both as ``\texttt{numeric}''. Consequently, we will also not worry about the specific categorization of numeric data and treat them as one. On the other hand, emphasis will be given to the difference between numeric and factors data.
...
As default, \texttt{R} saves variables that contain non-numeric values as factors. Otherwise, the variables are saved as numeric. The variable type is important because different statistical methods are applied to different data types. Hence, one should make sure that the variables that are analyzed have the appropriate type. Especially that factors using numbers to denote the levels are labeled as factors. Otherwise \texttt{R} will treat them as quantitative data.

\hypertarget{exercises-1}{%
\section{Exercises}\label{exercises-1}}

\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-22}{}{\label{exr:unnamed-chunk-22} }Consider the following relative frequency table on hurricanes that have made direct hits on the U.S. between 1851 and 2004 (\url{http://www.nhc.noaa.gov/gifs/table5.gif}). Hurricanes are given a strength category rating based on the minimum wind speed generated by the storm. Some of the entries to the table are missing.
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-23}{}{\label{exr:unnamed-chunk-23} }The number of calves that were born to some cows during their productive years was recorded. The data was entered into an R object by the name ``\texttt{calves}''. Refer to the following \texttt{R} code:
\EndKnitrBlock{exercise}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{freq <-}\StringTok{ }\KeywordTok{table}\NormalTok{(calves)}
\KeywordTok{cumsum}\NormalTok{(freq)}
\end{Highlighting}
\end{Shaded}
...
\item
  What is the relative frequency of cows that gave birth to at least 4 calves?
\end{enumerate}
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-27-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-28-1} \end{center}
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-39}{}{\label{exr:unnamed-chunk-39} }Three sequences of data were saved in 3 \texttt{R}
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-41}{}{\label{exr:unnamed-chunk-41} }The number of toilet facilities in 30 buildings
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-68-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-69-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-70-1} \end{center}
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-72}{}{\label{exr:unnamed-chunk-72} }A particular measles vaccine produces a reaction (a
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-73}{}{\label{exr:unnamed-chunk-73} }The Negative-Binomial distribution is yet another
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-112}{}{\label{exr:unnamed-chunk-112} }A subatomic particle hits a linear detector at random
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-131-1} \end{center}
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-142}{}{\label{exr:unnamed-chunk-142} }The data for the 50 patients is stored in file
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-143}{}{\label{exr:unnamed-chunk-143} }In Chapter~\ref{ChapTwoSamp} we will present a
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-154}{}{\label{exr:unnamed-chunk-154} }In Subsection~\ref{ComparingEstimators} we compare
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-155}{}{\label{exr:unnamed-chunk-155} }The goal in this exercise is to assess estimation of
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-172}{}{\label{exr:unnamed-chunk-172} }This exercise deals with an experiment that was
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-173}{}{\label{exr:unnamed-chunk-173} }Twenty observations are used in order to construct a
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-174}{}{\label{exr:unnamed-chunk-174} }Insurance companies are interested in knowing the
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-179-1} \end{center}
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-193}{}{\label{exr:unnamed-chunk-193} }Consider a medical condition that does not have a
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-194}{}{\label{exr:unnamed-chunk-194} }It is assumed, when constructing the \(t\)-test, that the
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-195}{}{\label{exr:unnamed-chunk-195} }Assume that you are interested in testing
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-198-1} \end{center}
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-205}{}{\label{exr:unnamed-chunk-205} }In this exercise we would like to analyze the results
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-208-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-221-1} \end{center}
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-222}{}{\label{exr:unnamed-chunk-222} }Figure~\ref{fig:Regression8} presents 10 points and
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-223}{}{\label{exr:unnamed-chunk-223} }Assume a regression model that describes the
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-224}{}{\label{exr:unnamed-chunk-224} }The file ``\texttt{aids.csv}'' contains data on the number of
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-225}{}{\label{exr:unnamed-chunk-225} }Below are the percents of the U.S. labor force
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-226}{}{\label{exr:unnamed-chunk-226} }Assume a regression model was fit to some data that
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-227}{}{\label{exr:unnamed-chunk-227} }In Chapter~\ref{ChapTwoSamp} we analyzed an example
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-232}{}{\label{exr:unnamed-chunk-232} }This exercise deals with a comparison between
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-233}{}{\label{exr:unnamed-chunk-233} }Cushing's syndrome disorder results from a tumor
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-235-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-240-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-241-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-249-1} \end{center}
...
\item
  The relative frequency of direct hits of category 4 or more is 0.0769. Observe that the cumulative relative of the value ``3'' is 0.6630 + 0.2601 = 0.9231. This follows from the fact
...
\def\labelenumi{\arabic{enumi}.}
\item
  The total number of cows that were involved in this study is 45. The object ``\texttt{freq}'' contain the table of frequency of the cows, divided according to the number of calves that they had. The cumulative frequency of all the cows that had 7 calves or less, which includes all cows in the study, is reported under the number ``7'' in the output of the expression ``\texttt{cumsum(freq)}''. This number is 45.
\item
  The number of cows that gave birth to a total of 4 calves is 10. Indeed, the cumulative frequency of cows that gave birth to 4 calves or less is 28. The cumulative frequency of cows
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-262-1} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-262-2} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-262-3} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-282-1} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-282-2} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-328-1} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-329-1} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-334-1} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{statthink_files/figure-latex/unnamed-chunk-336-1} \end{center}
